#  Exercise 5.7   #############################################################
#
# This program computes the number of channels by commercial ratio present 
# in the dataset.
#
###############################################################################
...
full_log.groupby("LogIdentifierID", "Undertaking_Name").agg(
    ...
    "commercial_ratio", F.round(F.col("duration_commercial") / F.col("duration_total"), 1)
).fillna(0).groupBy(
    "commercial_ratio").agg(F.count("LogIdentifierID").alias("number_of_channels")
).orderBy(
